...
for depth in data['depth'].unique():
    ...
    plt.scatter(depth_data['width'], depth_data['time'], s=10, alpha=0.5, label=f'Depth={depth}' if depth == data['depth'].unique()[0] else "")

# Plot the prediction lines
plt.plot(target_widths, linear_predictions, label='Linear Regression', marker='o', color='red')
plt.plot(target_widths, polynomial_predictions, label='Polynomial Regression (degree=2)', marker='x', color='green')
plt.plot(target_widths, exponential_predictions, label='Exponential Regression', marker='s', color='blue')
plt.plot(target_widths, poly2d_predictions, label='2D Polynomial Regression (degree=2)', marker='^', color='purple')
...
plt.ylabel('Time')
plt.title(f'Scaling Predictions and Original Data for Depth={target_depth}')
plt.legend()
plt.grid(True)
plt.yscale('log')
plt.show()
